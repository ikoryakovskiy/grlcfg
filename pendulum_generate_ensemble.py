...
def main():
    ...
    # Parameters
    runs = 1
    p = np.linspace(0, 49, 50)

    # Main
    rl_run_param(args, ["cfg/pendulum/sarsa_grid.yaml"], range(runs), p)
...
def rl_run_param(args, list_of_cfgs, runs, params):
    ...
    for cfg in list_of_cfgs:
        ...
        for p in params:
            str_param = int(round(p))

            for run in runs:
                # create local filename
                list_of_new_cfgs.append( "{}/{}-{:03d}-mp{}{}".format(loc, fname, str_param, run, fext) )

                # modify options
                conf['experiment']['environment']['model']['dynamics']['param'] = "cfg/pendulum/params.txt:{}".format(str_param)
                conf['experiment']['output'] = "{}-{:03d}-mp{}".format(fname, str_param, run)
                if "exporter" in conf['experiment']['environment']:
                  conf['experiment']['environment']['exporter']['file'] = "{}-{:03d}-mp{}".format(fname, str_param, run)
