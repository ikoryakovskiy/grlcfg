    ...
    for cfg in list_of_cfgs:
        ...
        for p in params:
            ...
            for run in runs:
                ...
                list_of_new_cfgs.append( "{}/{}-cf{:03d}-mp{}{}".format(loc, fname, str_param, run, fext) )

                # modify options
                conf['experiment']['environment']['xml'] = "../../leo/cfg/xm430_210_vc_leo_walk_cf{:03d}.xml".format(str_param)
                conf['experiment']['output'] = "{}-cf{:03d}-mp{}".format(fname, str_param, run)
                if "exporter" in conf['experiment']['environment']:
                  conf['experiment']['environment']['exporter']['file'] = "{}-cf{:03d}-mp{}".format(fname, str_param, run)
